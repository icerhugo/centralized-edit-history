...
中文（用户指南）：
- 总体介绍：
- 这是一个“集中式编辑历史”插件。它会在你编辑笔记时把每次变更压缩成条目并统一保存到 `/.obsidian/centralized-edit-history`。你可以在 Obsidian 内查看任意时刻的历史和差异；当笔记被资源管理器重命名/移动时，插件会自动接管原历史；孤立历史会被清理与“领养”，让历史始终跟随笔记而不需要你手工处理。
